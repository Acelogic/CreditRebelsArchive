...
\begin{tikzpicture}[scale=0.85, transform shape,
    ...
    every node/.style={draw, rounded corners, minimum width=1.8cm, minimum height=0.8cm, align=center, font=\small},
    edge from parent/.style={draw, ->, >=stealth}
]
\node[fill=primaryblue, text=white] {Credit Profile}
    child {node[fill=successbg] {Clean\\(No 60D+ late)}
        child {node[fill=lightgray] {Thick\\($\geq$4 TLs)}
            child {node[fill=tablerow2] {Mature\\($\geq$36 mo)}}
            child {node[fill=tablerow2] {Young\\(<36 mo)}}
        }
        child {node[fill=lightgray] {Thin\\(<4 TLs)}
            child {node[fill=tablerow2] {Mature}}
            child {node[fill=tablerow2] {Young}}
        }
    }
    child {node[fill=dangerbg] {Dirty\\(60D+ late)}
        child {node[fill=lightgray] {PR\\(Public Record)}
            child {node[fill=tablerow2] {Recent}}
            child {node[fill=tablerow2] {Mature}}
        }
        child {node[fill=lightgray] {No PR\\(Delinquency)}
            child {node[fill=tablerow2] {Recent}}
            child {node[fill=tablerow2] {Mature}}
        }
    };
\end{tikzpicture}
\caption{Score 8 Scorecard Segmentation Tree}
\end{figure}
...
\begin{tikzpicture}[scale=0.9]
    ...
    % Draw curve
    \draw[primaryblue, very thick] (0,5.5) -- (1,5) -- (3,4) -- (5,3) -- (7,2) -- (9,1) -- (10,0.3);
...
\begin{tikzpicture}[scale=0.9, transform shape]
    ...
    \node[draw=accentorange, fill=warningbg, rounded corners, font=\tiny, align=center] (note3) at (4.5,-6) {Use national BC\\with CL<\$31K};
...
\midrule
\rowcolor{tablerow2}
Primary debt factor & Utilization & AWB (Accounts w/ Balance) \\
\rowcolor{tablerow1}
...
\begin{tabular}{ll}
\textbf{Utilization (Aggregate)} & \threshold{<9.5\%} (or <4.5\% for max points) \\
...
\textbf{Mix} & 3--4 revolvers + 1 loan \\
\end{tabular}
\end{tcolorbox}
