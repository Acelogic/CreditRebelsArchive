...
Score 8 assigns you to one of \textbf{12 scorecards} based on your credit profile. The algorithm asks a series of questions to determine your scorecard:

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.85, transform shape]
    % Styles
    \tikzstyle{question} = [draw=primaryblue, fill=primaryblue!10, rounded corners, minimum width=3.5cm, minimum height=0.8cm, align=center, font=\small]
    \tikzstyle{answer} = [draw, rounded corners, minimum width=2.2cm, minimum height=0.6cm, align=center, font=\footnotesize]
    \tikzstyle{scorecard} = [draw, rounded corners, minimum width=1.8cm, minimum height=0.5cm, align=center, font=\scriptsize\bfseries]
    \tikzstyle{label} = [font=\scriptsize\itshape, text=mediumgray]
    \tikzstyle{arrow} = [draw, ->, >=stealth, thick]

    % === CLEAN PATH (Left Side) ===
    \node[question] (q1) at (-5,0) {Any 60+ day late\\in last 7 years?};
    \node[answer, fill=successbg] (clean) at (-5,-1.5) {NO $\rightarrow$ Clean};

    \node[question] (q2) at (-5,-3) {How many\\accounts?};
    \node[answer, fill=lightgray] (thick) at (-7,-4.5) {4+ $\rightarrow$ Thick};
    \node[answer, fill=lightgray] (thin) at (-3,-4.5) {<4 $\rightarrow$ Thin};

    \node[question] (q3a) at (-7,-6) {Age of oldest\\account?};
    \node[question] (q3b) at (-3,-6) {Age of oldest\\account?};

    \node[answer, fill=tablerow2] (tm) at (-8.5,-7.5) {3+ yrs};
    \node[answer, fill=tablerow2] (ty) at (-5.5,-7.5) {<3 yrs};
    \node[answer, fill=tablerow2] (thm) at (-4.5,-7.5) {3+ yrs};
    \node[answer, fill=tablerow2] (thy) at (-1.5,-7.5) {<3 yrs};

    \node[question] (q4a) at (-8.5,-9) {New revolver\\<12 mo?};
    \node[question] (q4b) at (-5.5,-9) {New revolver\\<12 mo?};
    \node[question] (q4c) at (-4.5,-9) {New revolver\\<12 mo?};
    \node[question] (q4d) at (-1.5,-9) {New revolver\\<12 mo?};

    % Clean scorecards (8 total)
    \node[scorecard, fill=successbg!40] at (-9.5,-10.5) {No};
    \node[scorecard, fill=successbg!40] at (-7.5,-10.5) {Yes};
    \node[scorecard, fill=successbg!40] at (-6.5,-10.5) {No};
    \node[scorecard, fill=successbg!40] at (-4.5,-10.5) {Yes};
    \node[scorecard, fill=successbg!40] at (-5.5,-10.5) {No};
    \node[scorecard, fill=successbg!40] at (-3.5,-10.5) {Yes};
    \node[scorecard, fill=successbg!40] at (-2.5,-10.5) {No};
    \node[scorecard, fill=successbg!40] at (-0.5,-10.5) {Yes};

    % === DIRTY PATH (Right Side) ===
    \node[answer, fill=dangerbg] (dirty) at (5,-1.5) {YES $\rightarrow$ Dirty};

    \node[question] (q2d) at (5,-3) {Bankruptcy, collections,\\or other public record?};
    \node[answer, fill=lightgray] (pr) at (3.5,-4.5) {YES $\rightarrow$ PR};
    \node[answer, fill=lightgray] (nopr) at (6.5,-4.5) {NO $\rightarrow$ No PR};

    \node[question] (q3d1) at (3.5,-6) {How recent?};
    \node[question] (q3d2) at (6.5,-6) {How recent?};

    % Dirty scorecards (4 total)
    \node[scorecard, fill=dangerbg!40] at (2.5,-7.5) {Recent};
    \node[scorecard, fill=dangerbg!40] at (4.5,-7.5) {Mature};
    \node[scorecard, fill=dangerbg!40] at (5.5,-7.5) {Recent};
    \node[scorecard, fill=dangerbg!40] at (7.5,-7.5) {Mature};
    ...
    % Arrows - Dirty path
    \draw[arrow] (dirty) -- (q2d);
    \draw[arrow] (q2d) -- (pr);
    \draw[arrow] (q2d) -- (nopr);
    \draw[arrow] (pr) -- (q3d1);
    \draw[arrow] (nopr) -- (q3d2);

    % Labels
    \node[font=\small\bfseries, text=accentgreen] at (-5,-11.3) {8 Clean Scorecards};
    \node[font=\small\bfseries, text=accentred] at (5,-8.3) {4 Dirty Scorecards};

\end{tikzpicture}
\caption{Score 8 Scorecard Assignment: Which of the 12 Scorecards Are You On?}
\end{figure}
